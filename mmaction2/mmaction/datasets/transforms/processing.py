...
class ThreeCrop(BaseTransform):
    ...
    def transform(self, results):
        ...
        if crop_w < img_w and crop_h < img_h:
            # Calculate both horizontal and vertical steps
            w_step = (img_w - crop_w) // 2
            h_step = (img_h - crop_h) // 2
            offsets = [
                (0, 0),  # top-left
                (2 * w_step, 0),  # top-right
                (w_step, 0),  # top-middle
                (0, 2 * h_step),  # bottom-left
                (2 * w_step, 2 * h_step),  # bottom-right
                (w_step, 2 * h_step),  # bottom-middle
                (0, h_step),  # middle-left
                (2 * w_step, h_step),  # middle-right
                (w_step, h_step),  # center
            ]
        elif crop_w < img_w:
            # Only calculate horizontal steps
            w_step = (img_w - crop_w) // 2
            offsets = [
                (0, 0),  # left
                (2 * w_step, 0),  # right
            ...
            # If the crop size is larger than the image dimensions, raise an error or handle it
